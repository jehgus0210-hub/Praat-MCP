...
음성언어학 분석 도구 **Praat**을 Claude 같은 LLM에서 자연어로 바로 실행할 수 있게 연결해주는 MCP(Model Context Protocol) 서버입니다.

## 목차

1. [이게 뭐예요?](#1-이게-뭐예요)
2. [준비물 (한 번만 설치)](#2-준비물한-번만-설치)
3. [폴더 구성](#3-폴더-구성)
4. [.env 파일 만들기 (필수)](#4-env-파일-만들기필수)
5. [Claude Desktop에 연결하기](#5-claude-desktop에-연결하기)
6. [사용 방법 (실제 분석)](#6-사용-방법실제-분석)
...
프로젝트 폴더(`mcp-praat`) 안에 `.env` 파일을 만듭니다. 확장자 없이 파일 이름이 `.env` 입니다.
...
| `PRAAT_EXE_DIR` | `Praat.exe`가 있는 폴더 경로 (또는 `Praat.exe` 전체 경로도 동작) |
| `SCRIPT_DIR` | `.praat` 스크립트 폴더 경로 |
| `RESULT_DIR` | 결과 `.txt`가 저장될 폴더 경로 |
...
- Praat가 설치된 위치가 다르면 `PRAAT_EXE_DIR`만 본인 PC에 맞게 바꾸면 됩니다.
- `RESULT_DIR`에 `\result`처럼 경로를 중복으로 넣지 마세요. 위 예시처럼 한 번만 쓰면 됩니다.

## 5. Claude Desktop에 연결하기

Claude Desktop 설정 파일(`claude_desktop_config.json`)을 수정해서, Claude가 이 프로그램을 실행하도록 연결합니다.

**주의**
- `--directory`에는 반드시 프로젝트 폴더의 절대경로가 들어가야 합니다.
- Windows 경로의 `\`는 JSON 문자열 안에서 `\\`로 한 번 더 escape 해야 합니다.
...
```json
{
  "mcpServers": {
    "praat-mcp": {
      ...
      "args": [
        "--directory",
        "C:\\Users\\Your-Name\\Desktop\\mcp-praat",
        "run",
        "main.py"
      ]
    }
